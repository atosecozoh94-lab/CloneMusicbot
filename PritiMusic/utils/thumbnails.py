...
# ✅ Main bot thumbnail only
# Clone database / clone owner logic intentionally removed.
# This file does not import clonebotdb, so it will not modify or depend on clone bot data.
...
def get_random_fallback_img():
    """Return fallback thumbnail URL if generation fails."""
    if YOUTUBE_IMG_URL:
...
async def get_thumb(videoid, user_id=None, client=None):
    """
    Main bot thumbnail generator.

    Keep signature compatible with old calls:
        get_thumb(videoid, user_id, client)

    Clone-specific database/owner logic is removed to avoid clone side effects.
    """
    bot_name = "MUSIC BOT"
    ...
    try:
        if client:
            ...
            bot_name = (me.first_name or "MUSIC BOT").upper()
    except Exception:
        bot_name = "MUSIC BOT"

    # ✅ Main bot cache filename only. No bot_id / clone_id added.
    filename = os.path.join(CACHE_DIR, f"{videoid}.png")
    ...
    try:
        ...
        # Top bot name only. Owner/clone text removed.
        ...
        draw.text(
            (panel_x1 + 36, panel_y1 + 225),
            artist_text,
            font=artist_font,
            fill=(210, 215, 225, 235),
        )

        views_text = trim_to_width(f"Views • {views}", meta_font, title_max_width)
        draw.text(
            (panel_x1 + 36, panel_y1 + 270),
            views_text,
            font=meta_font,
            fill=(170, 180, 195, 235),
        )
        ...
        kdraw.ellipse(
            (knob_x - 16, knob_y - 16, knob_x + 16, knob_y + 16),
            fill=(255, 255, 255, 255),
        )
        ...
        draw.text(
            (bar_x + bar_w - total_w, bar_y + 28),
            total_time,
            font=time_font,
            fill=(240, 240, 245, 235),
        )

        # Footer
        footer_text = "High Quality Streaming Experience"
        draw.text(
            (panel_x1 + 36, panel_y2 - 42),
            footer_text,
            font=footer_font,
            fill=(160, 175, 190, 220),
        )
